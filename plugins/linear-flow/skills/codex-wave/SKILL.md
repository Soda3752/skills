...
**其他 wave skill 讓實作者自己寫實作紀錄，這裡做不到——所以要刻意保留 Codex 的思考軌跡。** 從它的回傳裡把這三件事撈出來寫進註解：它**考慮過但沒選的做法**、它**撞到的牆**（原始錯誤訊息）、它**標了沒驗的項目**。這些是轉述時最先被砍掉的東西，偏偏是日後最有用的——下一個接手的人第一個冒出來的念頭，往往正是 Codex 已經試過並放棄的方案。撈不到就明講「Codex 未交代」，不要因為缺這段就整段省略。
