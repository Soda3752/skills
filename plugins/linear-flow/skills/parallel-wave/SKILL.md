...
合併後在票上補一則**整合註解**，標題 `## 整合與複查（主控）`。**agent 那則回答「當時是怎麼想的」，你這則回答「現在能不能信」——不要把它的實作紀錄重講一遍**，那只會讓票變長而資訊沒增加。

**寫法照 `.claude/linear-workflow.md` 的「註解怎麼寫」**，正文照結論在前的順序：

1. **這張票現在是什麼狀態**：已進哪個分支、能不能信。一句話
2. **我對 agent 判斷的修正**：哪裡它想錯了、我改了什麼。沒這段就只是複讀
3. **複查抓到的真問題**：每條一行（抓到什麼 → 怎麼處置）
4. **仍未驗證**：**原樣保留 agent 標的未驗證項**，不要因為合併成功就把「未實機驗證」升級成通過
5. 下游影響（有才寫）

**這些進 `<details>` 摺疊區，不要放正文**：rebase 前後的 commit hash（rebase 會改寫 hash，只寫一個日後對不上）、在 base 分支上實跑的逐條驗證結果與**測試實際數量**、複查發現的逐條推導。**你派工時犯的錯不要寫進票**，那是流程問題，該進對話或 log。
