...
- 派 wave / loop 一定要附**授權段**。

### 授權段：為什麼非寫不可

`herdr-claude-wave` 與 `herdr-codex-wave` 都是單波審慎版，流程本身就寫著「每波開始前停下來與使用者確認範圍」。派到另一個 pane 之後，那個「使用者」變成沒人——它做完一波就停在原地問「下一波要派幾張」，而派工的人已經交手走了，整批票卡在半路。

這不是那兩支 skill 的 bug，是派工少了授權。授權段要講四件事：

- 第一波開工前把波次範圍問清楚，等確認再開工。
- 第一波之後不再問，波次大小、worktree 怎麼切、何時整合全部自決，做到看板上沒有可動的 Todo 為止。
- 只有兩種情況才需要問：這件事沒有合理預設可選（猜錯整批重做）、或決定會破壞既有功能 / 動到未授權的東西。
- **問題不准擋住推進**：要問就列成清單貼出來，同一則訊息裡繼續開下一波，只把真正受該決策影響的票留下不派。除非整波每一張都卡在同一個未決事項，才可以整批停。
- 每波收工貼一次進度就**直接接著做，不要等回話**，而且**回報結尾不准是問句**。

後兩條是實測打出來的補丁。只寫「規格歧義才問」的話，agent 會把任何開放式設計決定歸進歧義，然後把整波停下來——即使那個決定只擋得住其中一張票。實際形狀：一波做完列出三項待決事項，其中只有一項擋住一張票、另外三張完全可動，它仍然停在原地問「要我直接開嗎」。授權段要把「問」和「停」拆開：問可以，停不行。

指揮 pane 也要用 `herdr agent start ... -- --permission-mode auto` 起。沒有的話它會為了 `git worktree add`、`cp -Rc` 這類日常指令一路跳確認，然後停著等一個不會來的人。不要升級成 `--dangerously-skip-permissions`：指揮 pane 的 cwd 旁邊就是主 repo。
